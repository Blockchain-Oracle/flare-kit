...
importLine: "import { useAttestation } from '@flarekit-dev/react'"
...
<PropsTable rows={[
  ...
  { name: 'error', type: 'SerializedError | undefined', description: 'Typed, with its recovery class and evidence — never a bare string. Set by a refused request, or by a reconciliation read that threw a FlareKitError. A reader that throws anything else is swallowed silently — wrap a hand-written reader so its failures surface as FlareKitError.' },
