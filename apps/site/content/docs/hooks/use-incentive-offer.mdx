...
importLine: "import { useIncentiveOffer } from '@flare-kit/react'"
...
`@flare-kit/core`. The price formula is fitted to four live measurements rather
...
<PropsTable rows={[
  ...
  { name: 'chainId', type: 'number', required: true, description: 'Which deployment. The manager address comes from @flare-kit/contracts.' },
