...
importLine: "import { OPERATION_STATES, canTransition, pathTo } from '@flare-kit/core'"
prev: { href: '/docs/headless/lifecycle', label: 'The operation lifecycle' }
next: { href: '/docs/headless/adapters', label: 'Adapters' }
---
...
```ts
import { OPERATION_STATES, type OperationState } from '@flare-kit/core'
...
```ts
import { isTerminal, isSuccess } from '@flare-kit/core'
...
```ts
import { canTransition } from '@flare-kit/core'
...
`action_required`, or it fails. And `submitted` cannot go back to `executing` —
once something is broadcast, the operation moves forward or waits, but it never
pretends the broadcast did not happen.
...
```ts
import { applyTransition } from '@flare-kit/core'
...
- **`same_state`** — the target equals the current state. Evidence *and* the
  patch are applied, which is what makes a repeated reconcile idempotent
  rather than inert.
...
```ts
import { pathTo } from '@flare-kit/core'

pathTo('ready', 'succeeded') // the intermediate states, in order
...
one state. Analytics names `submitted` as a funnel boundary and never counts it
as a conversion to success. The direct mint reaches `succeeded` only from
