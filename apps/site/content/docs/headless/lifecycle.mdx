...
importLine: "import type { OperationRecord } from '@flare-kit/core'"
prev: { href: '/docs/hooks/operation-store', label: 'Operation store' }
next: { href: '/docs/headless/states', label: 'States and transitions' }
---

Everything `@flare-kit/core` does to a mint, a redeem, a swap, a bridge send, a
...
```ts
import { createOperation } from '@flare-kit/core'
...
```ts
import { DIRECT_MINT_STEPS, advanceSteps } from '@flare-kit/core'
...
```ts
import { availableActions, isAvailable } from '@flare-kit/core'
